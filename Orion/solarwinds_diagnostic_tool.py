...
class ResolutionFinder:
    ...
    def search_solarwinds_kb(self, error_message: str, category: str) -> List[Dict]:
        """Search SolarWinds KB, Success Center, Thwack and general web"""
        all_kb_articles = []
        ...
        try:
            ...
            if not search_terms or len(search_terms.strip()) < 3:
                logging.warning("Search terms too short, skipping KB search")
                return []
    ...
    def _search_success_center(self, search_terms: str) -> List[Dict]:
        ...
        try:
            ...
            logging.info(f"Searching Success Center...")
    ...
    def _search_thwack(self, search_terms: str) -> List[Dict]:
        ...
        try:
            ...
            logging.info(f"Searching Thwack...")
    ...
    def _search_web(self, search_terms: str) -> List[Dict]:
        """Search general web for SolarWinds articles"""
        articles = []
        try:
            search_query = f"{search_terms} solarwinds"
            search_url = f"https://www.google.com/search?q={requests.utils.quote(search_query)}"
            
            logging.info(f"Searching web...")
        ...
        if key_terms:
            ...
            for t in key_terms:
                if t.lower() not in seen:
                    seen.add(t.lower())
                    unique.append(t)
            search_query = ' '.join(unique[:5])
        else:
            words = message.split()
            meaningful = [w for w in words if len(w) > 3 and not w.isdigit()]
            search_query = ' '.join(meaningful[:10])
